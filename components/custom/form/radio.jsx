import { GENDER_FEMALE, GENDER_MALE } from "@/utils/data";
        ...
        <div className="grid grid-cols-2 gap-1 bg-white rounded-full p-1">
          {genders?.map((gender) => (
            ...
            >
              ...
              />
              <span className="gender__text peer-checked:text-white text-primary">
                {gender?.title}
              </span>
  ...
  if (page == "profile") {
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        <div className="grid grid-cols-2 gap-1 bg-bg-2 rounded-lg p-1 border border-bg-3">
          {genders?.map((gender) => (
            <label
              key={gender.id}
              className="gender__item flex items-center justify-center gap-2 cursor-pointer py-3 relative z-0 rounded-lg transition-colors duration-200"
            >
              <input
                type="radio"
                name={name}
              ...
              <div className="absolute inset-0 rounded bg-white peer-checked:bg-primary transition-colors duration-200 -z-10"></div>
            </label>
          ))}
        </div>
        {errors?.message && (
